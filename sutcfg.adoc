...
[NOTE]
`itrigger` and `etrigger` are excluded because Sdtrig fires them just before the first instruction of the trap handler, and each `tdata2` bit selects a cause number interpreted in the mode in which that handler executes. The S/HS and VS views can filter those bits with `mideleg`/`medeleg` or `hideleg`/`hedeleg`, as specified in <<sstcfg-type-constraints>>. U-mode and VU-mode do not execute trap handlers and have no equivalent delegation register, so this interface does not define a writable subset of `tdata2`.
...
[NOTE]
See the Sscsrind and Sucsrind specifications cite:[indirectcsr] for how bit 60 (CSRIND) in `mstateen0` and `hstateen0` restricts access to `siselect` and `sireg*` from privilege modes less privileged than M-mode, including VS-mode when V=1. Access to `uiselect` and `uireg*` is gated by the UCSRIND bit (bit 4) of `mstateen0`, `hstateen0`, and `sstateen0`, as specified in <<sucsrind, Sucsrind>>. The CSRIND bit provides blanket control over the Sscsrind indirect CSR infrastructure, while TR provides trigger-specific delegation control.
