...
At a high level, the Debug Module Security Extension provides platform-level controls, including `psecdbgen`, to enable or bypass the security constraints. The Sdsec ISA extension provides per-hart debug and trace enable controls, including platform-controlled `mdbgen` and `mtrcen` states for M-mode and lower-privilege enable fields in `mdtcfg`. Together, these controls determine whether external debug or trace is allowed at the hart's current privilege mode.
