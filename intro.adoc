...
- When `pdbgsecen`=1 and `mdbgen`=1, external debug is allowed for all privilege modes.
- When `pdbgsecen`=1, `mdbgen`=0, and `SEDBGEN`=1 (if implemented), external debug is allowed for S/HS-mode and lower privilege modes.
- When `pdbgsecen`=1, `mdbgen`=0, `SEDBGEN`=0, and `VSEDBGEN`=1 (if implemented), external debug is allowed for VS-mode and VU-mode.
- When `pdbgsecen`=1, `mdbgen`=0, `SEDBGEN`=0, and `UEDBGEN`=1 (if implemented), external debug is allowed for U-mode. `VSEDBGEN` and `VUEDBGEN` do not affect U-mode debug permission.
- When `pdbgsecen`=1, `mdbgen`=0, `SEDBGEN`=0, `VSEDBGEN`=0, and `VUEDBGEN`=1 (if implemented), external debug is allowed for VU-mode. `UEDBGEN` does not affect VU-mode debug permission.
