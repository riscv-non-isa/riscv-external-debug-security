...
[NOTE]
For M-mode firmware debugging with protected ROM code, assert `mdbgen` only after the ROM code is complete. For example, the `mdbgen` of the non-RoT hart can be controlled by RoT. The `mdbgen` of the RoT hart, if present, can be managed by external hardware like OTP/life-cycle fuse, with optional authentication. Establishing a chain of trust (CoT) allows M-mode firmware debugging without exposing ROM code to unauthorized debugger.
