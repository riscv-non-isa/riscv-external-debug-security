...
The `sdcsr.dmprv` modifies the effective *debug access privilege* to memory in S-mode debugger. When `sdcsr.dmprv`=0, the effective *debug access privilege* to memory follows <<dbgpriv>>; when `sdcsr.dmprv`=1, the effective *debug access privilege* to memory is represented by `spp` and `spv` fields in `sstatus`.

`sdcsr.dmprv` is 0 when `mdbgen`=1.
