...
The debug operations listed below are affected by Sdsec extension. Within the context of this chapter, *debug operations* refer specifically to those listed, while other operations are excluded.
...
[NOTE]
A pending request to enter debug mode might be dynamically changed from disallowed state to allowed state due to the update of debug controls. For example, the software finished the confidential code and grants external debugger to debug. Afterwards, it spins in a while(1) loop, waiting for debugger to take control and jump out of the loop. 

[[mdbgctl]]
==== Machine Mode Debug Control

An input port, named mdbgen[i], is introduced to control the debuggability of machine mode for each hart i. This signal is propagated to the hart i and Debug Module. When mdbgen[i] is set to 1, the <<dbops, debug operations>> are permitted when hart i executes in all modes. Moreover the following rules apply:
...
The Smsdedbg extension introduces `sdedbgalw` field in CSR <<Sdseccsr,msdcfg>> within hart i to control debuggability of supervisor domains cite:[smmtt]. The <<dbops, debug operations>> are allowed when hart i executes in the supervisor domain only if the logical-OR of values in `sdedbgalw` and mdbgen[i] is 1.
...
The <<dbops, debug operations>>  are always disallowed for hart i when the logical-OR of `sdedbgalw` and mdbgen[i] is 0. In this case, the hart follows the <<dbgdisallowed, behaviros>> in all modes. 
