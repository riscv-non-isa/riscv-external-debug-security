...
When `mdbgen`=0 and `sdedbgalw`=1, M-mode interrupts during single-stepping in S-mode behave as follows. The hart exits Debug Mode and traps to M-mode. It then executes the trap handler, returns to S-mode, executes the single-stepped instruction, and finally re-enters Debug Mode.
