...
The supervisor domain debug of hart i is determined by both mdbgen[i] and the `sdedbgalw` field of CSR <<Sdseccsr,msdcfg>> within hart i. Debug operations listed in <<dbops>> are allowed when hart i executes in the supervisor domain only if the logical-OR of values in sdedbgalw and mdbgen[i] is 1.
...
[[dbgpriv]]
[options="header"]
.Debuggable privilege levels per debug controls 
|============================================
...
| scontext(0x5a8)  | Always     | Debug access privilege >= S
| hcontext(0x6a8)  | Always     | Debug access privilege >= S
