...
When `mdbgen`=0, `sdedbgalw`=1, and `dcsr.step`=1, if a single-stepped instruction in a debug-allowed supervisor domain transfers control to a debug-disallowed M-mode trap handler, the hart will execute the handler in M-mode and re-enter Debug Mode immediately after an `mret` instruction returns to the supervisor domain (i.e., `mret` with `mstatus.mpp` < 3). The hart shall not re-enter Debug Mode if the `mret` instruction returns to a debug-disallowed supervisor domain (i.e., `sdedbgalw`=0).
