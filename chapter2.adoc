...
When `mdbgen`=0, `sdedbgalw`=1, and `dcsr.step`=1, a single-stepped instruction in a debug-allowed supervisor domain transfers control to a debug-disallowed M-mode trap handler. The hart will execute the handler in M-mode and re-enter Debug Mode immediately after an `mret` instruction returns to the supervisor domain (i.e., `mret` with `mstatus.mpp` < 3). The hart shall not re-enter Debug Mode if the `mret` instruction returns to a debug-disallowed supervisor domain (i.e., `sdedbgalw`=0). This could be caused by the trap handler returning to a different supervisor domain or the trap handler clearing `sdedbgalw` to 0 before returning to the same supervisor domain. 

Note that the M-mode trap handler might restart the instruction (e.g., due to page faults or interrupts), which can prevent the single-step from advancing past that instruction. To assist users, the external debugger should perform an additional step if the PC does not change after a regular step.
