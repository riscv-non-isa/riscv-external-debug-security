...
Some trigger CSRs, tselect, tdata1, tdata2, tdata3, and tinfo are always accessible in Debug Mode, remaining CSRs, tcontrol, scontext, hcontext, mcontext, and mscontext continue to adhere to the <<dbgaccpriv, debug access privilege>>.

//what does permitted mean?
//ahdere => requires M mode privilege? Need to reword this line, or be explicit?
