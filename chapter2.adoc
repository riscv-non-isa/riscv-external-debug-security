...
The `sdcsr.dmprv` modifies the effective *debug access privilege* to memory in S-mode debugger. When `sdcsr.dmprv`=0, the effective *debug access privilege* to memory follows <<dbgpriv>>; when `sdcsr.dmprv`=1, the effective *debug access privilege* to memory is represented by `sstatus.spp` and `sstatus.spv`. 

`sdcsr.dmprv` only takes effect when `mdbgen` is 0, `sdcsr.dmprv` is 0 when `mdbgen` is 1.

The effective *debug access privilege* to memory in M-mode debugger can be modified by `dcsr.mprven` and `mstatus.mprv` as specified in The RISC-V Debug Specification cite:[dbgspec]. 

With Sdsec, `dcsr.mprven` only takes effect When `mdbgen`=1, `dcsr.mprven` is 0 when `mdbgen`=0.
...
1: In Debug Mode, the privilege level for load and store operations is modified and indicated by `sstatus.spp` and `sstatus.spv`.| WARL   | 0      
|=================================================================================================================================================================
...
The `sdcsr` and `sdpc` registers provide supervisor read/write access to the `dcsr` and `dpc` registers respectively. Moreover, the `sdcsr` adds `dmprv` to modify effective *debug access privilege* in S-mode. Both registers are only accessible in Debug Mode. 
...
[NOTE]
The `nmip`, `mprven`, `stoptime`, `stopcount`, `ebreakm` and `cetrig` fields in `dcsr` are configurable only by M-mode, they are masked in `sdcsr`, while `prv[1]` is hardwired to 0 `sdcsr`. The field for `mprven` is reclaimed by `dmprv` in `sdcsr` layout to avoid waste of fields. 
