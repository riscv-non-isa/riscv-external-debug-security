...
The field `aamvirtual` in the command (at 0x17 in the Debug Module) determines whether the Access Memory uses a physical or virtual address. When a physical memory access is issued (`aamvirtual`=0), the hart must check whether it is permitted to conduct such access as described in <<phyaddr>>. If it is not allowed, the hart responds with an exception, and the Debug Module will set `cmderr` to 3 and clear the data registers to 0.
