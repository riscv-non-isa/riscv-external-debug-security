...
System Bus Access must be checked by bus initiator protection mechanisms such as IOPMP cite:[iopmp], WorldGuard cite:[worldguard]. Bus protection unit may return error to Debug Module on illegal access, in that case, Debug Module will set xyz.sberror to 6 (security fault).
