...
[NOTE] Disallowing the Quick Access command for M-mode simplifies the implementation, as it avoids the need for special handling of Quick Access. Consequently, Quick Access is inhibited for debuggers that are only allowed to debug supervisor domains. 
