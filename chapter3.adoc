...
This chapter outlines the security enhancements implemented in the Debug Module. It explains how the <<dbops, debug operations>> from external debuggers will be impacted when the external debug is disallowed by <<mdbgctl, Machine Mode Debug Control>> and/or <<submdbgctl, Supervisor Domain Debug Control>>.
...
The hart can only be halted and enter debug mode when it is running in privilege levels specified in <<dbgpriv>>. Otherwise, the halt request will remain pending. Consequently, the response to the external debugger request may take longer than one second, and this constraint must be eliminated.
