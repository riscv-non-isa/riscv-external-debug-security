...
[NOTE] 
Quick Access abstract commands effect a halt, execution of Program Buffer, and resume of the selected hart. These halts cannot remain pending until debug is allowed because the debugger blocks while waiting for completion. To handle this, the hart would need to distinguish between Quick Access and other halt requests. Since Quick Access is an optional optimization, the implementation avoids this extra hardware complexity by disallowing Quick Access when `mdbgen` is 0.
...
Error status bits are internally maintained for each hart. The `allsecfault` and `anysecfault` fields in `dmstatus` indicate the error status of the currently selected harts. These error statuses are sticky and can only be cleared by writing 1 to `acksecfault` in `dmcs2`.
