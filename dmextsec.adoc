...
The platform state element `psecdbgen` is introduced to enable the debug security constraints defined by this specification. When `psecdbgen` is 0, the platform retains full debugging capabilities, as if the Secure Debug extensions were not implemented:
...
If `mdtcfg` is implemented, it remains accessible to M-mode software when `psecdbgen` is 0. Implemented debug-related fields retain their specified WARL behavior, but their values do not constrain external debug; the unrestricted debug behavior defined above applies. The trace-related fields remain independent of `psecdbgen` and retain their specified effect.

When `psecdbgen` is 1, the debug security constraints in this specification apply.
...
| ALLSECURED  | The field is 0 when `psecdbgen` is 0. When `psecdbgen` is 1, this field is 1 if all currently selected harts implement Smmedbgsec.         | R      | -
| ANYSECURED  | The field is 0 when `psecdbgen` is 0. When `psecdbgen` is 1, this field is 1 if any currently selected hart implements Smmedbgsec.         | R      | -
