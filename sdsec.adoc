...
[NOTE]
Debug and trace extensions can be implemented independently. For example, a system may implement Smsdedbg without Smsdetrc, or vice versa.
...
| Smmdedbg                              | mdbgen                       | mdtcfg        | M-mode
| Smsdedbg                              | mdtcfg.SEDBGEN               | sdcsr, sdpc   | S/HS-mode
| Smvsdedbg                             | mdtcfg.VSEDBGEN              | None          | VS-mode
| Smudedbg                              | mdtcfg.UEDBGEN               | udcsr, udpc   | U-Mode or VU-mode
|=====================================================================
...
| SEDBGEN    | S/HS-mode external debug allowed. When set to 1, external debug is allowed for S/HS-mode, U-mode, VS-mode, and VU-mode when `mdbgen` is 0. Introduced by Smsdedbg.  | WARL   | 0
| VSEDBGEN   | VS-mode external debug allowed. When set to 1, external debug is allowed for VS-mode and VU-mode when `mdbgen` is 0 and `SEDBGEN` is 0. Introduced by Smvsdedbg.  | WARL   | 0
| UEDBGEN    | U-mode/VU-mode external debug allowed. When set to 1, U-mode or VU-mode external debug is allowed when `mdbgen`=0, `SEDBGEN`=0, and `VSEDBGEN`=0. Introduced by Smudedbg.  | WARL   | 0
|=================================================================================================================================================================
...
The Smsdedbg extension is an optional extension available when Smmdedbg is implemented. It introduces the `SEDBGEN` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls S/HS-mode debuggability when M-mode external debug is disallowed.
...
The Smvsdedbg extension is an optional extension available when Smsdedbg is implemented. It introduces the `VSEDBGEN` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls VS-mode debuggability when S/HS-mode external debug is disallowed.
...
| Smsdetrc           | mdtcfg.SETRCEN      | S/HS-mode
| Smvsdetrc          | mdtcfg.VSETRCEN     | VS-mode
| Smudetrc           | mdtcfg.UETRCEN      | U-Mode or VU-mode
|================================================
...
The following `mdtcfg` fields provide lower-privilege trace control states. The `mdtcfg` CSR must be implemented when Smmdetrc is implemented, even if Smmdedbg is not implemented.
...
|=================================================================================================================================================================
| Field       | Description                                                                                                  | Access | Reset
| SETRCEN    | S/HS-mode external trace allowed. When set to 1, external trace is allowed for S/HS-mode, U-mode, VS-mode, and VU-mode when `mtrcen` is 0. Introduced by Smsdetrc.  | WARL   | 0
| VSETRCEN   | VS-mode external trace allowed. When set to 1, external trace is allowed for VS-mode and VU-mode when `mtrcen` is 0 and `SETRCEN` is 0. Introduced by Smvsdetrc.  | WARL   | 0
| UETRCEN    | U-mode/VU-mode external trace allowed. When set to 1, U-mode or VU-mode trace is allowed when `mtrcen`=0, `SETRCEN`=0, and `VSETRCEN`=0. Introduced by Smudetrc.  | WARL   | 0
|=================================================================================================================================================================

==== M-mode Trace Security Extension (Smmdetrc)

The Smmdetrc extension is the mandatory base extension when trace is implemented.
...
The Smsdetrc extension is an optional extension available when Smmdetrc is implemented. It introduces the `SETRCEN` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls S/HS-mode trace output when M-mode trace is disallowed.
...
The Smvsdetrc extension is an optional extension available when Smsdetrc is implemented. It introduces the `VSETRCEN` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls VS-mode trace output when S/HS-mode trace is disallowed.
...
The Smudetrc extension is an optional extension available when Smmdetrc or Smsdetrc or Smvsdetrc is implemented. It introduces the `UETRCEN` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls U-mode or VU-mode (when virtualization mode is 1) trace output when M-mode or S/HS-mode or VS-mode trace is disallowed.
