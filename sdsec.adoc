...
The optional Smsdedbg extension is provided to control S/HS-mode external debug operations when M-mode external debug is disallowed. It introduces the `SDEDBGALW` field (bit 7) in CSR <<Sdseccsr,`msdcfg`>>, and it controls S/HS-mode external debug operations.
...
- Privilege-changing instructions (other than EBREAK) executed in the Program Buffer must either act as a NOP or raise an exception (stopping execution and setting `abstractcs`.CMDERR to 3). 
- The new `sdcsr` and `sdpc` (<<smodecsr>>) are introduced in Smsdedbg to access `dcsr` and `dpc` in S/HS-mode.
...
The optional Smvsdedbg extension is provided to control VS-mode external debug operations when S/HS-mode external debug is disallowed. It introduces the `VSEDBGALW` field (bit TBD) in CSR <<Sdseccsr,`msdcfg`>>, and it controls VS-mode external debug operations.
...
- Privilege-changing instructions (other than EBREAK) executed in the Program Buffer must either act as a NOP or raise an exception (stopping execution and setting `abstractcs`.CMDERR to 3). 
- The `sdcsr` and `sdpc` (from <<smodecsr>> in Smsdedbg) provide VS-mode read/write access to `dcsr` and `dpc` registers respectively as specified in <<vsmodecsr>>.
...
The optional Smudedbg extension is provided to control U-mode and VU-mode external debug operations when S/HS-mode and VS-mode external debug are disallowed. It introduces the `USEDBGALW` field (bit TBD) in CSR <<Sdseccsr,`msdcfg`>>, and it controls U-mode and VU-mode external debug operations.
...
- Privilege-changing instructions (other than EBREAK) executed in the Program Buffer must either act as a NOP or raise an exception (stopping execution and setting `abstractcs`.CMDERR to 3). 
- The `udcsr` and `udpc` provide U-mode or VU-mode read/write access to `dcsr` and `dpc` registers respectively as specified in <<umodecsr>>.
...
The optional Smsdetrc extension is provided to control S/HS-mode trace operations when M-mode trace is disallowed. It introduces the `SDETRCALW` field (bit 8) in CSR <<Sdseccsr,`msdcfg`>>, and it controls S/HS-mode trace output.
...
The optional Smvsdetrc extension is provided to control VS-mode trace operations when S/HS-mode trace is disallowed. It introduces the `VSETRCALW` field (bit TBD) in CSR <<Sdseccsr,`msdcfg`>>, and it controls VS-mode trace output.
...
The optional Smudetrc extension is provided to control U-mode and VU-mode trace operations when S/HS-mode and VS-mode trace are disallowed. It introduces the `USETRCALW` field (bit TBD) in CSR <<Sdseccsr,`msdcfg`>>, and it controls U-mode and VU-mode trace operations.
