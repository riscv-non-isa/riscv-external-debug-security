...
* **Smmsecedbg (Base)**: Mandatory base extension providing M-mode external debug control
* **Smssecedbg (Optional)**: Extends Smmsecedbg to add S/HS-mode external debug control
* **Smvssecedbg (Optional)**: Extends Smmsecedbg to add VS-mode external debug control
* **Smusecedbg (Optional)**: Extends Smmsecedbg to add U-mode and VU-mode external debug control

When trace is implemented, the following trace control extensions are defined:

* **Smmsecetrc (Base)**: Mandatory base extension providing M-mode trace control
* **Smssecetrc (Optional)**: Extends Smmsecetrc to add S/HS-mode trace control
* **Smvssecetrc (Optional)**: Extends Smmsecetrc to add VS-mode trace control
* **Smusecetrc (Optional)**: Extends Smmsecetrc to add U-mode and VU-mode trace control

The Smmsecedbg base extension is mandatory when external debug is implemented. The Smmsecetrc base extension is mandatory when trace is implemented. If a system implements both external debug and trace, both base extensions must be implemented. Implementations may choose to implement some or all of the optional extensions depending on their system requirements. When implementing extensions for lower privilege modes, all extensions for higher privilege modes of the same type (external debug or trace) must also be implemented. These extensions work together to establish a hierarchical external debug and trace control where external debug access or trace output can be restricted to specific privilege levels.

[NOTE]
Debug and trace extensions can be implemented independently. For example, a system may implement Smssecedbg without Smssecetrc, or vice versa.
...
| Smmsecedbg                              | mdbgen                       | mdtcfg        | M-mode
| Smssecedbg                              | mdtcfg.SEDBGEN               | sdcsr, sdpc   | S/HS-mode
| Smvssecedbg                             | mdtcfg.VSEDBGEN              | None          | VS-mode
| Smusecedbg                              | mdtcfg.UEDBGEN               | udcsr, udpc   | U-Mode or VU-mode
|=====================================================================

When external debug is allowed for a privilege mode, it is allowed for all lower privilege modes as well. If an optional debug extension is not implemented, the corresponding control state behaves as 0; for lower-privilege modes, the `mdtcfg` field representing that control state may either be absent or read-only 0.
...
The following fields in `mdtcfg` provide lower-privilege external debug control states. The trace control states are described in <<sdsectrc>>.
...
| SEDBGEN    | S/HS-mode external debug allowed. When set to 1, external debug is allowed for S/HS-mode, U-mode, VS-mode, and VU-mode when `mdbgen` is 0. Introduced by Smssecedbg.  | WARL   | 0
| VSEDBGEN   | VS-mode external debug allowed. When set to 1, external debug is allowed for VS-mode and VU-mode when `mdbgen` is 0 and `SEDBGEN` is 0. Introduced by Smvssecedbg.  | WARL   | 0
| UEDBGEN    | U-mode/VU-mode external debug allowed. When set to 1, U-mode or VU-mode external debug is allowed when `mdbgen`=0, `SEDBGEN`=0, and `VSEDBGEN`=0. Introduced by Smusecedbg.  | WARL   | 0
|=================================================================================================================================================================
...
|=====================================================================================================
| mdbgen | SEDBGEN (if implemented)   | VSEDBGEN (if implemented)  | UEDBGEN (if implemented)   | Debug Access Privilege
...
.Maximum Allowed Resume Privilege Mode
|=====================================================================================================
| mdbgen | SEDBGEN (if implemented)   | VSEDBGEN (if implemented)  | UEDBGEN (if implemented)   | Maximum Debug Allowed Privilege on resume
| 1      | Don't care                  | Don't care                  | Don't care                  | M-Mode                                         
| 0      | 1                           | Don't care                  | Don't care                  | S/HS-Mode                                  
| 0      | 0                           | 1                           | Don't care                  | VS-Mode
| 0      | 0                           | 0                           | 1                           | U-Mode or VU-Mode
...
[[mdbgctl]]
==== M-mode External Debug Security Extension (Smmsecedbg)

The Smmsecedbg extension is the mandatory base extension when external debug is implemented.
...
[[smsdbgctl]]
==== S/HS-mode External Debug Security Extension (Smssecedbg)

The Smssecedbg extension is an optional extension available when Smmsecedbg is implemented. It introduces the `SEDBGEN` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls S/HS-mode debuggability when M-mode external debug is disallowed.
...
- Privilege-changing instructions (other than EBREAK) executed in the Program Buffer must raise an exception, stopping execution and setting `abstractcs`.CMDERR to 3.
- The new `sdcsr` and `sdpc` (<<smodecsr>>) are introduced in Smssecedbg to access `dcsr` and `dpc` by an S/HS-mode debugger.
...
[[effectivedbgpriv]]
===== Debug Access Privilege to memory in Smssecedbg
...
==== VS-mode External Debug Security Extension (Smvssecedbg)

The Smvssecedbg extension is an optional extension available when Smssecedbg is implemented. It introduces the `VSEDBGEN` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls VS-mode debuggability when S/HS-mode external debug is disallowed.
...
- Privilege-changing instructions (other than EBREAK) executed in the Program Buffer must raise an exception, stopping execution and setting `abstractcs`.CMDERR to 3.
- The `sdcsr` and `sdpc` (from Smssecedbg) provide VS-mode read/write access to `dcsr` and `dpc` registers respectively as specified in <<vsmodecsr>>.
...
[[vseffectivedbgpriv]]
===== Debug Access Privilege to memory in Smvssecedbg
...
==== U-mode and VU-mode External Debug Security Extension (Smusecedbg)

The Smusecedbg extension is an optional extension available when Smmsecedbg or Smssecedbg or Smvssecedbg is implemented. It introduces the `UEDBGEN` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls U-mode or VU-mode (when virtualization mode is 1) debuggability when M-mode or S/HS-mode or VS-mode external debug are disallowed.

The `UEDBGEN` field only takes effect when both `mdbgen`=0 and `SEDBGEN`=0 and `VSEDBGEN`=0.

When `UEDBGEN` is set to 1, U-mode or VU-mode (when virtualization mode is 1) external debug is allowed:
...
- Privilege-changing instructions (other than EBREAK) executed in the Program Buffer must raise an exception, stopping execution and setting `abstractcs`.CMDERR to 3.
- The `udcsr` and `udpc` are introduced in Smusecedbg to provide U-mode or VU-mode read/write access to `dcsr` and `dpc` registers respectively, as specified in <<umodecsr>>.
- The <<maxdbgpriv, maximum allowed resume privilege mode>> is U-mode or VU-mode.

When `UEDBGEN` is set to 0, external debug is disallowed for all modes:
...
| Smmsecetrc           | mtrcen              | M-mode
| Smssecetrc           | mdtcfg.SETRCEN      | S/HS-mode
| Smvssecetrc          | mdtcfg.VSETRCEN     | VS-mode
| Smusecetrc           | mdtcfg.UETRCEN      | U-Mode or VU-mode
|================================================

When trace is allowed for a privilege mode, it is allowed for all lower privilege modes as well. If an optional trace extension is not implemented, the corresponding control state behaves as 0; for lower-privilege modes, the `mdtcfg` field representing that control state may either be absent or read-only 0.
...
The following `mdtcfg` fields provide lower-privilege trace control states. The `mdtcfg` CSR must be implemented when Smmsecetrc is implemented, even if Smmsecedbg is not implemented.
...
|=================================================================================================================================================================
| Field       | Description                                                                                                  | Access | Reset
| SETRCEN    | S/HS-mode external trace allowed. When set to 1, external trace is allowed for S/HS-mode, U-mode, VS-mode, and VU-mode when `mtrcen` is 0. Introduced by Smssecetrc.  | WARL   | 0
| VSETRCEN   | VS-mode external trace allowed. When set to 1, external trace is allowed for VS-mode and VU-mode when `mtrcen` is 0 and `SETRCEN` is 0. Introduced by Smvssecetrc.  | WARL   | 0
| UETRCEN    | U-mode/VU-mode external trace allowed. When set to 1, U-mode or VU-mode trace is allowed when `mtrcen`=0, `SETRCEN`=0, and `VSETRCEN`=0. Introduced by Smusecetrc.  | WARL   | 0
|=================================================================================================================================================================

==== M-mode Trace Security Extension (Smmsecetrc)

The Smmsecetrc extension is the mandatory base extension when trace is implemented.
...
==== S/HS-mode Trace Security Extension (Smssecetrc)

The Smssecetrc extension is an optional extension available when Smmsecetrc is implemented. It introduces the `SETRCEN` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls S/HS-mode trace output when M-mode trace is disallowed.
...
==== VS-mode Trace Security Extension (Smvssecetrc)

The Smvssecetrc extension is an optional extension available when Smssecetrc is implemented. It introduces the `VSETRCEN` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls VS-mode trace output when S/HS-mode trace is disallowed.
...
==== U-mode and VU-mode Trace Security Extension (Smusecetrc)

The Smusecetrc extension is an optional extension available when Smmsecetrc or Smssecetrc or Smvssecetrc is implemented. It introduces the `UETRCEN` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls U-mode or VU-mode (when virtualization mode is 1) trace output when M-mode or S/HS-mode or VS-mode trace is disallowed.

The `UETRCEN` field only takes effect when both `mtrcen`=0 and `SETRCEN`=0 and `VSETRCEN`=0.

When `UETRCEN` is set to 1, trace is allowed for U-mode or VU-mode. When `UETRCEN` is set to 0, trace is inhibited for all privilege modes by asserting the logical `sec_inhibit` signal to the trace encoder.
