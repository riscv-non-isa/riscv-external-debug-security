...
| Smvsdedbg          | msdcfg.VSEDBGALW | msdcfg.VSETRCALW | vsdcsr        | VS-mode
...
The `sdcsr` and `sdpc` (see <<smodecsr>>) are accessible in Debug Mode if `SDEDBGALW`=1; otherwise, the access will fail and `abstractcs`.CMDERR is set to 3 (exception). When external debug is disallowed in S/HS-mode, the configuration visible in `sdcsr` will be ignored as if it were 0.
