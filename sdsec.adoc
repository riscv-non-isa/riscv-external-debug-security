...
The Smsedbgsec extension is optional and requires Smmedbgsec. It introduces the `SEDBGEN` field in CSR <<mdtcfgreg,`mdtcfg`>> and controls S/HS-mode debuggability when M-mode external debug is disallowed.
...
The Smvsedbgsec extension is optional and requires Smsedbgsec. It introduces the `VSEDBGEN` field in CSR <<mdtcfgreg,`mdtcfg`>> and controls VS-mode debuggability when S/HS-mode external debug is disallowed.
