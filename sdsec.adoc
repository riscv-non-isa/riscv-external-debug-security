...
The Sdsec extension provides debug control across different RISC-V privilege levels through a mandatory M-mode debug control and a set of optional extensions. This allows selectively enabling or disabling external debug capabilities for specific privilege modes.

The debug controls include:

* **M-mode Debug Control**: Mandatory control for external debug access to M-mode
* **S-mode Debug Control (Smsdedbg)**: Optional extension controlling external debug access to S/HS-mode  
* **VS-mode Debug Control (Smvsdedbg)**: Optional extension controlling external debug access to VS-mode
* **U-mode Debug Control (Smudedbg)**: Optional extension controlling external debug access to U/VU-mode

The table below lists the debug control and CSRs added for each extension:

[[dbgctlcsr]]
[cols="20%,20%,20%,20%,20%", options="header"]
.Debug Control and CSRs
|===========================================
|               |     Sdsec(baseline)   |   Smsdedbg(optional)      |    Smvsdedbg(optional)  |  Smudedbg(optional)   
| Debug Control |     mdbgen            |   SDEDBGALW               |    VSEDBGALW            |  USEDBGALW
| CSR           |     N/A               |   sdcsr, sdpc             |    vsdcsr               |  udcsr, udpc
|===========================================
...
The following table summarizes the valid extension combinations:

[[validimplcomb]]
[cols="20%,80%", options="header"]
.Valid Extension Combinations by Architecture
|===========================================
| Architecture |          Valid implementations           
| M            |   Sdsec only                    
| M/U          |   Sdsec or Sdsec + Smudedbg         
| M/S/U        |   Sdsec, Sdsec + Smsdedbg, Sdsec + Smsdedbg + Smudedbg                     
| M/S/VS/VU/U  |   Sdsec, Sdsec + Smsdedbg, Sdsec + Smsdedbg + Smvsdedbg, Sdsec + Smsdedbg + Smvsdedbg + Smudedbg                 
|===========================================
