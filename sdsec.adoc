...
The `udcsr` and `udpc` <<umodecsr>>, are introduced in Smudedbg. They are accessible in Debug Mode if `USEDBGALW`=1. When external debug is disallowed in U-mode or VS-mode (`USEDBGALW`=0), the configuration visible in `udcsr` will be ignored as if it were 0.
