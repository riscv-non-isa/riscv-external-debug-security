...
The following table defines the external debug control states and associated CSRs introduced by the external debug security extensions. In this specification, _control state_ covers platform-level control state, such as `psecdbgen`, per-hart platform-controlled states, such as `mdbgen`, and CSR fields, such as `mdtcfg.SEDBGEN`.
...
| Smmdedbg                              | psecdbgen                    | None          | Platform/all modes
