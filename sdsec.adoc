...
| Smsdedbg           | mdtcfg.SEDBGALW           | sdcsr, sdpc   | S/HS-mode  
| Smvsdedbg          | mdtcfg.VSEDBGALW          | None          | VS-mode
| Smudedbg           | mdtcfg.UEDBGALW           | udcsr, udpc   | U-Mode or VU-mode
|=====================================================================

[[mdtcfg_reg]]
The `mdtcfg` (Machine Debug and Trace Configuration) is a machine-mode read/write CSR (address TBD) introduced by this specification. It holds configuration fields for external debug and trace security control. The `mdtcfg` CSR is independent of the `msdcfg` CSR defined by the Smsdid extension; implementations that do not support Smsdid/SMMTT can implement `mdtcfg` without requiring `msdcfg` or `mmpt`. If an optional extension is not implemented, the corresponding field is read-only 0.
...
....
{reg: [
  {bits:  1, name:  'SEDBGALW'},
  {bits:  1, name:  'VSEDBGALW'},
  {bits:  1, name:  'UEDBGALW'},
  {bits:  5, name:  'WPRI', type: 1},
  {bits:  1, name:  'SETRCALW'},
  {bits:  1, name:  'VSETRCALW'},
  {bits:  1, name:  'UETRCALW'},
  {bits: 21, name:  'WPRI', type: 1},
], config:{lanes: 3, hspace:1024}}
....

The following fields in `mdtcfg` control external debug security. The trace security fields are described in <<sdsectrc>>.

.Details of external debug fields in `mdtcfg`
[options="header"]
[cols="15%,55%,15%,15%"]
|=================================================================================================================================================================
| Field       | Description                                                                                                  | Access | Reset
| SEDBGALW    | S/HS-mode external debug allowed. When set to 1, external debug is allowed for S/HS-mode, U-mode, VS-mode, and VU-mode when `mdbgen` is 0. Introduced by Smsdedbg.  | WARL   | 0
| VSEDBGALW   | VS-mode external debug allowed. When set to 1, external debug is allowed for VS-mode and VU-mode when `mdbgen` is 0 and `SEDBGALW` is 0. Introduced by Smvsdedbg.  | WARL   | 0
| UEDBGALW    | U-mode/VU-mode external debug allowed. When set to 1, U-mode or VU-mode external debug is allowed when `mdbgen`=0, `SEDBGALW`=0, and `VSEDBGALW`=0. Introduced by Smudedbg.  | WARL   | 0
|=================================================================================================================================================================
...
|=====================================================================================================
| mdbgen | SEDBGALW (if implemented)   | VSEDBGALW (if implemented)  | UEDBGALW (if implemented)   | Debug Access Privilege
...
.Maximum Allowed Resume Privilege Mode
|=====================================================================================================
| mdbgen | SEDBGALW (if implemented)   | VSEDBGALW (if implemented)  | UEDBGALW (if implemented)   | Maximum Debug Allowed Privilege on resume 
| 1      | Don't care                  | Don't care                  | Don't care                  | M-Mode                                         
| 0      | 1                           | Don't care                  | Don't care                  | S/HS-Mode                                  
| 0      | 0                           | 1                           | Don't care                  | VS-Mode
| 0      | 0                           | 0                           | 1                           | U-Mode or VU-Mode
...
The Smsdedbg extension is an optional extension available when Smmdedbg is implemented. It introduces the `SEDBGALW` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls S/HS-mode debuggability when M-mode external debug is disallowed.

The `SEDBGALW` field only takes effect when `mdbgen` is 0.

When `SEDBGALW` is set to 1, S/HS-mode external debug is allowed:
...
When `SEDBGALW` is set to 0 and the hart is running in S/HS-mode, external debug is disallowed for S/HS-mode:
...
When `SEDBGALW` is set to 0 and the hart is running in a debug-allowed lower privilege mode (e.g, U-mode), the following restrictions apply:

- Single-stepping cannot stop in S/HS-mode.
- Interrupts delegated to S/HS-mode cannot be disabled by setting `dcsr`.STEPIE=0. 

[NOTE]
When `SEDBGALW`=0 and `sdcsr`.STEP=1, a single-stepped instruction in a debug-allowed privilege mode may transfer control to the S/HS-mode trap handler. The hart will execute the handler in S/HS-mode and re-enter Debug Mode immediately after an SRET instruction returns to the debug-allowed privilege mode (i.e., SRET with `sstatus`.SPP=0). The hart does not re-enter Debug Mode if the SRET instruction returns to a debug-disallowed privilege mode.

[[smodecsr]]
===== `sdcsr` and `sdpc` 

When `SEDBGALW` is 1, the `sdcsr` and `sdpc` registers provide S/HS-mode read/write access to the `dcsr` and `dpc` registers respectively. However, `sdcsr` does not expose access to the `MPRVEN` field; instead, it repurposes the `MPRVEN` bit position with a `DMPRV` field to modify the *effective debug access privilege* in S/HS-mode. Both registers are only accessible in Debug Mode.
...
The Smvsdedbg extension is an optional extension available when Smsdedbg is implemented. It introduces the `VSEDBGALW` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls VS-mode debuggability when S/HS-mode external debug is disallowed.

The `VSEDBGALW` field only takes effect when both `mdbgen` is 0 and `SEDBGALW` is 0.
...
The `sdcsr`.DMPRV modifies the *effective debug access privilege* of loads and stores for a VS-mode debugger when `SEDBGALW` is 0 and `VSEDBGALW` is 1.
...
When `UEDBGALW` is set to 0, external debug is disallowed for all modes: 
...
| Smsdetrc           | mdtcfg.SETRCALW      | S/HS-mode  
| Smvsdetrc          | mdtcfg.VSETRCALW     | VS-mode
| Smudetrc           | mdtcfg.UETRCALW      | U-Mode or VU-mode
|================================================

The following fields in <<mdtcfgreg,`mdtcfg`>> control trace security. If an optional extension is not implemented, the corresponding field is read-only 0. The `mdtcfg` CSR must be implemented when Smmdetrc is implemented, even if Smmdedbg is not implemented.

.Details of trace security fields in `mdtcfg`
[options="header"]
[cols="15%,55%,15%,15%"]
|=================================================================================================================================================================
| Field       | Description                                                                                                  | Access | Reset
| SETRCALW    | S/HS-mode external trace allowed. When set to 1, external trace is allowed for S/HS-mode, U-mode, VS-mode, and VU-mode when `mtrcen` is 0. Introduced by Smsdetrc.  | WARL   | 0
| VSETRCALW   | VS-mode external trace allowed. When set to 1, external trace is allowed for VS-mode and VU-mode when `mtrcen` is 0 and `SETRCALW` is 0. Introduced by Smvsdetrc.  | WARL   | 0
| UETRCALW    | U-mode/VU-mode external trace allowed. When set to 1, U-mode or VU-mode trace is allowed when `mtrcen`=0, `SETRCALW`=0, and `VSETRCALW`=0. Introduced by Smudetrc.  | WARL   | 0
|=================================================================================================================================================================
...
The Smsdetrc extension is an optional extension available when Smmdetrc is implemented. It introduces the `SETRCALW` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls S/HS-mode trace output when M-mode trace is disallowed.

The `SETRCALW` field only takes effect when `mtrcen` is 0.

When `SETRCALW` is set to 1, trace is allowed for all privilege modes except M-mode. When `SETRCALW` is set to 0, trace is inhibited for S/HS-mode by asserting the logical `sec_inhibit` signal to the trace encoder when the hart is executing in S/HS-mode.

==== VS-mode Trace Security Extension (Smvsdetrc)

The Smvsdetrc extension is an optional extension available when Smsdetrc is implemented. It introduces the `VSETRCALW` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls VS-mode trace output when S/HS-mode trace is disallowed.

The `VSETRCALW` field only takes effect when both `mtrcen` is 0 and `SETRCALW` is 0.
...
The Smudetrc extension is an optional extension available when Smmdetrc or Smsdetrc or Smvsdetrc is implemented. It introduces the `UETRCALW` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls U-mode or VU-mode (when virtualization mode is 1) trace output when M-mode or S/HS-mode or VS-mode trace is disallowed.

The `UETRCALW` field only takes effect when both `mtrcen`=0 and `SETRCALW`=0 and `VSETRCALW`=0.

When `UETRCALW` is set to 1, trace is allowed for U-mode or VU-mode. When `UETRCALW` is set to 0, trace is inhibited for all privilege modes by asserting the logical `sec_inhibit` signal to the trace encoder.
