...
The following tables summarize the external debug and trace control signals/fields and associated CSRs:
...
The tables below show valid implementation combinations for different architectures:
...
[[Sdseccsr]]
=== Debug and Trace Control CSR

The CSR `msdcfg`, which holds the external debug and trace control fields (<<dbgctlcsr>> and <<trcctlcsr>>), is defined in the RISC-V Supervisor Domains Access Protection specification cite:[smmtt]. The Smsdedbg and/or Smsdetrc extensions must be implemented to support security control for external debugging and/or tracing in the corresponding privilege modes.
...
[[mdbgctl]]
==== M-mode External Debug and Trace Control (Smmdedbg, Smmdetrc)

The Smmdedbg extension is the mandatory base extension when external debug is implemented. The Smmdetrc extension is the mandatory base extension when trace is implemented. These extensions introduce security controls for M-mode external debug and trace operations respectively. Two state elements are defined in each hart:

* `mdbgen`: Controls M-mode external debug operations (Smmdedbg extension, required when external debug is implemented)
* `mtrcen`: Controls M-mode trace output (Smmdetrc extension, required when trace is implemented)

When external debug or trace is allowed for a privilege mode, it is allowed for all lower privilege modes as well. The Smmdedbg and Smmdetrc extensions operate independently - a privilege mode can have external debug enabled while trace is disabled, or vice versa. If an optional extension is not implemented, its control state will be treated as if it were 0; the control state may either be not implemented or read-only 0.  
...
[[smsdbgctl]]
==== S/HS-mode External Debug and Trace Control (Smsdedbg, Smsdetrc)

The optional Smsdedbg and Smsdetrc extensions introduce the following fields in CSR <<Sdseccsr,`msdcfg`>>:

* `SDEDBGALW` (bit 7): Controls S/HS-mode external debug operations (Smsdedbg extension)
* `SDETRCALW` (bit 8): Controls S/HS-mode trace output (Smsdetrc extension)

The `SDEDBGALW` field only takes effect when `mdbgen` is 0; otherwise, debug control is dominated by the Smmdedbg extension. The `SDETRCALW` field only takes effect when `mtrcen` is 0; otherwise, trace control is dominated by the Smmdetrc extension.
...
==== VS-mode External Debug and Trace Control (Smvsdedbg, Smvsdetrc)

The optional Smvsdedbg and Smvsdetrc extensions introduce the following fields in CSR <<Sdseccsr,`msdcfg`>>:

* `VSEDBGALW` (bit TBD): Controls VS-mode external debug operations (Smvsdedbg extension)
* `VSETRCALW` (bit TBD): Controls VS-mode trace output (Smvsdetrc extension)

The `VSEDBGALW` field only takes effect when both `mdbgen` and `SDEDBGALW` are 0; otherwise, debug control is dominated by the Smmdedbg or Smsdedbg extensions. The `VSETRCALW` field only takes effect when both `mtrcen` and `SDETRCALW` are 0; otherwise, trace control is dominated by the Smmdetrc or Smsdetrc extensions.
...
==== U-mode External Debug and Trace Control (Smudedbg, Smudetrc)

The optional Smudedbg and Smudetrc extensions introduce the following fields in CSR <<Sdseccsr,`msdcfg`>>:

* `USEDBGALW` (bit TBD): Controls U/VU-mode external debug operations (Smudedbg extension)
* `USETRCALW` (bit TBD): Controls U/VU-mode trace output (Smudetrc extension)

The `USEDBGALW` field only takes effect when all higher privilege mode debug controls are 0; otherwise, debug control is dominated by higher privilege debug extensions. The `USETRCALW` field only takes effect when all higher privilege mode trace controls are 0; otherwise, trace control is dominated by higher privilege trace extensions.

===== U-mode External Debug Control

The specific behavior of `USEDBGALW` depends on the architecture:

* In M/U architecture: `USEDBGALW` takes effect when `mdbgen`=0
* In M/S/U architecture: `USEDBGALW` takes effect when both `mdbgen`=0 and `SDEDBGALW`=0
* In M/S/VS/VU/U architecture: `USEDBGALW` takes effect when `mdbgen`=0, `SDEDBGALW`=0, and `VSEDBGALW`=0 (for VU-mode) or when `mdbgen`=0 and `SDEDBGALW`=0 (for U-mode)
...
The specific behavior of `USETRCALW` depends on the architecture:

* In M/U architecture: `USETRCALW` takes effect when `mtrcen`=0
* In M/S/U architecture: `USETRCALW` takes effect when both `mtrcen`=0 and `SDETRCALW`=0
* In M/S/VS/VU/U architecture: `USETRCALW` takes effect when `mtrcen`=0, `SDETRCALW`=0, and `VSETRCALW`=0 (for VU-mode) or when `mtrcen`=0 and `SDETRCALW`=0 (for U-mode)
