...
The optional Smsdedbg extension introduces the `SDEDBGALW` field (bit 7) in CSR <<Sdseccsr,`msdcfg`>>. The `SDEDBGALW` field only takes effect when `mdbgen` is 0; otherwise, debug control is dominated by `mdbgen`.

[NOTE]
All behavior described in this section applies only when `mdbgen` is 0. When `mdbgen` is 1, M-mode debug control overrides all lower privilege mode settings. Moreover, when S/HS-mode debug control is in effect, it implies that M-mode debug control in <<mdbgctl>> is also in effect as `mdbgen` is 0.
...
The `sdcsr` and `sdpc` (see <<smodecsr>>) are accessible in Debug Mode if `SDEDBGALW`=1. When external debug is disallowed in S/HS-mode, the configuration visible in `sdcsr` will be ignored as if it were 0.

[[smodecsr]]
====== `Sdcsr` and `sdpc`

The `sdcsr` and `sdpc` registers provide S/HS-mode read/write access to the `dcsr` and `dpc` registers respectively. Moreover, the `sdcsr` adds DMPRV to modify the *effective debug access privilege* in S/HS-mode. Both registers are only accessible in Debug Mode. 
...
The optional Smvsdedbg extension introduces the `VSEDBGALW` field (bit 8) in CSR <<Sdseccsr,`msdcfg`>>. The `VSEDBGALW` field only takes effect when both `mdbgen` and `VSEDBGALW` are 0; otherwise, debug control is dominated by `mdbgen` or `VSEDBGALW`.

[NOTE]
All behavior described in this section applies only when both `mdbgen` and `VSEDBGALW` are 0. When `mdbgen` is 1 or `VSEDBGALW` is 1, M-mode debug control or S/HS-mode debug control overrides all lower privilege mode settings. Moreover, when VS-mode debug control is in effect, it implies that M-mode and S/HS-mode debug control in <<mdbgctl>> and <<smsdbgctl>> is also in effect as `mdbgen` and `VSEDBGALW` are 0.
...
// TODO: add CSR description with vs-mode debug control.
//When `VSEDBGALW` is 1, the `sdcsr` and `sdpc` registers provide VS-mode read/write access to the `dcsr`, the   
...
// TODO: move it to s/vs/u mode control.
===== Debug Access Privilege to memory
