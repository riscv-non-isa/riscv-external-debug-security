...
| Smsdedbg           | mdtcfg.SDEDBGALW          | sdcsr, sdpc   | S/HS-mode  
| Smvsdedbg          | mdtcfg.VSEDBGALW          | None          | VS-mode
| Smudedbg           | mdtcfg.USEDBGALW          | udcsr, udpc   | U-Mode or VU-mode
|=====================================================================
...
| SDEDBGALW   | S/HS-mode external debug allowed. When set to 1, S/HS-mode external debug is allowed when `mdbgen` is 0. Introduced by Smsdedbg.  | WARL   | 0
| VSEDBGALW   | VS-mode external debug allowed. When set to 1, VS-mode external debug is allowed when `mdbgen` is 0 and `SDEDBGALW` is 0. Introduced by Smvsdedbg.  | WARL   | 0
| USEDBGALW   | U-mode/VU-mode external debug allowed. When set to 1, U-mode or VU-mode external debug is allowed when `mdbgen`=0, `SDEDBGALW`=0, and `VSEDBGALW`=0. Introduced by Smudedbg.  | WARL   | 0
|=================================================================================================================================================================
...
|=====================================================================================================
| mdbgen | SDEDBGALW (if implemented)  | VSEDBGALW (if implemented)  | USEDBGALW (if implemented)  | Debug Access Privilege
...
.Maximum Allowed Resume Privilege Mode
|=====================================================================================================
| mdbgen | SDEDBGALW (if implemented)  | VSEDBGALW (if implemented)  | USEDBGALW (if implemented)  | Maximum Debug Allowed Privilege on resume 
| 1      | Don't care                  | Don't care                  | Don't care                  | M-Mode                                         
| 0      | 1                           | Don't care                  | Don't care                  | S/HS-Mode                                  
| 0      | 0                           | 1                           | Don't care                  | VS-Mode
| 0      | 0                           | 0                           | 1                           | U-Mode or VU-Mode
...
The Smsdedbg extension is an optional extension available when Smmdedbg is implemented. It introduces the `SDEDBGALW` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls S/HS-mode debuggability when M-mode external debug is disallowed.

The `SDEDBGALW` field only takes effect when `mdbgen` is 0.
...
The `VSEDBGALW` field only takes effect when both `mdbgen` is 0 and `SDEDBGALW` is 0.
...
| Smsdetrc           | mdtcfg.SDETRCALW     | S/HS-mode  
| Smvsdetrc          | mdtcfg.VSETRCALW     | VS-mode
| Smudetrc           | mdtcfg.USETRCALW     | U-Mode or VU-mode
|================================================

The following fields in <<mdtcfgreg,`mdtcfg`>> control trace security. If an optional extension is not implemented, the corresponding field is read-only 0.
...
| USETRCALW   | U-mode/VU-mode external trace allowed. When set to 1, U-mode or VU-mode trace is allowed when `mtrcen`=0, `SDETRCALW`=0, and `VSETRCALW`=0. Introduced by Smudetrc.  | WARL   | 0
|=================================================================================================================================================================
...
The Smsdetrc extension is an optional extension available when Smmdetrc is implemented. It introduces the `SDETRCALW` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls S/HS-mode trace output when M-mode trace is disallowed.
...
The Smudetrc extension is an optional extension available when Smmdetrc or Smsdetrc or Smvsdetrc is implemented. It introduces the `USETRCALW` field in CSR <<mdtcfgreg,`mdtcfg`>>, and controls U-mode or VU-mode (when virtualization mode is 1) trace output when M-mode or S/HS-mode or VS-mode trace is disallowed.
