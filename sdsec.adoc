...
| 0      | 1      | All except M                      | Next level privilege mode
| 0      | 0      | None                              | N/A
|============================================

[NOTE]
The next level privilege mode is the next level after M-mode. If S-mode is implemented, this is S-mode. If only M-mode and U-mode are implemented (no S-mode), it is U-mode.
...
The `dcsr`, `dpc`, and `dscratch0/1` are accessible in Debug Mode only if `mdbgen`=1; otherwise, the access will fail and `abstractcs`.CMDERR is set to 3 (exception). The `sdcsr`, `udcsr`, `sdpc`, and `udpc` are always accessible in Debug Mode.
...
User shadow of Debug Mode CSRs is not supported when S-mode is implemented.
...
[NOTE]
When M-mode enables debugging for the supervisor domain, it can optionally delegate the triggers to the supervisor domain, allowing an external debugger with next level privilege mode to configure these triggers.
