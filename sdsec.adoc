...
[[mdbgctl]]
==== M-mode External Debug Control (Smmdedbg)

The Smmdedbg extension is the mandatory base extension when external debug is implemented. A state element in each hart, named `mdbgen`, is introduced to control the debuggability of M-mode.
...
[[smsdbgctl]]
==== S/HS-mode External Debug Control (Smsdedbg)

The optional Smsdedbg extension introduces the `SDEDBGALW` field (bit 7) in CSR <<Sdseccsr,`msdcfg`>>. This field controls S/HS-mode external debug operations.
...
==== VS-mode External Debug Control (Smvsdedbg)

The optional Smvsdedbg extension introduces the `VSEDBGALW` field (bit TBD) in CSR <<Sdseccsr,`msdcfg`>>. This field controls VS-mode external debug operations.

The `VSEDBGALW` field only has effect when both `mdbgen` and `SDEDBGALW` are 0; otherwise, debug control is dominated by the Smmdedbg or Smsdedbg extensions.
...
==== U-mode External Debug Control (Smudedbg)

The optional Smudedbg extension introduces the `USEDBGALW` field (bit TBD) in CSR <<Sdseccsr,`msdcfg`>>. This field controls U/VU-mode external debug operations.
...
When trace is implemented, trace output can expose sensitive information across privilege boundaries. Trace data may reveal execution flow, data values, memory addresses, and other implementation details. The Sdsec trace extensions provide security controls to restrict trace output based on privilege levels.
...
==== S/HS-mode Trace Control (Smsdetrc)

The optional Smsdetrc extension introduces the `SDETRCALW` field (bit 8) in CSR <<Sdseccsr,`msdcfg`>>. This field controls S/HS-mode trace output.
...
==== VS-mode Trace Control (Smvsdetrc)

The optional Smvsdetrc extension introduces the `VSETRCALW` field (bit TBD) in CSR <<Sdseccsr,`msdcfg`>>. This field controls VS-mode trace output.
...
==== U-mode Trace Control (Smudetrc)

The optional Smudetrc extension introduces the `USETRCALW` field (bit TBD) in CSR <<Sdseccsr,`msdcfg`>>. This field controls U/VU-mode trace output.
