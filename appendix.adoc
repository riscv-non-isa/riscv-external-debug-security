...
- When `psecdbgen`=1 and all higher privilege controls are 0 and `UEDBGEN`=1 (if implemented), external debug is allowed for U-mode or VU-mode (when virtualization mode is 1) only.
- When `psecdbgen`=1 and all debug enable controls are 0, external debug is completely disallowed.
...
- When all higher privilege controls are 0 and `UETRCEN`=1 (if implemented), trace is allowed for U-mode or VU-mode (when virtualization mode is 1) only; trace is inhibited for all higher privilege modes
