...
M-mode software is responsible for managing the `SDEDBGALW`, `VSEDBGALW`, and `USEDBGALW` fields in the `msdcfg` CSR for external debug, and the `SDETRCALW`, `VSETRCALW`, and `USETRCALW` fields in the `msdcfg` CSR for trace. The `mdbgen` and `mtrcen` states are typically controlled by platform hardware (RoT, lifecycle fuses) and are read-only from software perspective.
...
3. Context-switch debug/trace security policies when switching between supervisor softwares 
...
1. Call to M-mode to apply the debug/trace security policy before launching an application or VM.  
2. Provide interfaces to handle dynamic debug/trace security policy update requests (e.g., SBI calls from VMs) and call to M-mode to apply the security policy accordingly.
...
1. Call to S/HS-mode to apply the debug/trace security policy before launching an application.  
